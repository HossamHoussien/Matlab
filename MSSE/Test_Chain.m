function  [result,correctCount,incorrectCount] = Test_Chain(T,S)
FVs = importdata('FeatureVectors.mat','featureVectors1');
correctCount = 0;
incorrectCount = 0;
...
for img = 1 : length(srcFiles)
    ...
    %GET DIFFERENCE
    index = getDifference(feature,FVs);
    num = Labels(index);
    if ((num+1) == img)
