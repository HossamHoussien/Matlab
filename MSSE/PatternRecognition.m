function [featureVectors1,featureVectors2]= PatternRecognition (T,S)
featureVectors1 = [];
featureVectors2 = [];

%READ ALL FILES IN THAT PATH
directory = strcat('Samples\','*.bmp');
...
for img = 1 : length(srcFiles)
    feature = zeros(T*S,8);
    %READ IMAGE
    filename = strcat('Samples\',srcFiles(img).name);
    im = imread(filename);
    ...
    featureVectors1 = [featureVectors1;feature];
    
    % TO BE USED IN MSSE
    [Fr,Fc] = size(feature);
    temp = reshape(feature',[1,Fr*Fc]);
    featureVectors2 = [featureVectors2;temp];
    
end
save('FeatureVectors.mat','featureVectors1');
end
